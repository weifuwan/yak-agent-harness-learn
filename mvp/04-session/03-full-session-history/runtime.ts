...
function decideNextStep(message: AssistantMessage): LoopDecision {
  ...
  if (toolCalls.length > 1) {
    throw new Error(
      `session:03 intentionally supports exactly one tool call per step, but received ${toolCalls.length}`,
    )
  }
...
export async function runAgentWithFullSession(
  ...
  // 从这一轮开始，Session 本身就是完整的运行历史。
  // 每个真正发生的 user / assistant(tool_call) / tool(result) / assistant(final)
  // 都在发生时立即写入 Session，而不是只在最终回答时补一份聊天摘要。
