...
function decideNextStep(message: AssistantMessage): LoopDecision {
  ...
  if (toolCalls.length > 1) {
    throw new Error(
      `session:04 intentionally supports exactly one tool call per step, but received ${toolCalls.length}`,
    )
  }
...
export async function runAgentInSession(
  ...
  const session = getOrCreateSession(options.sessionStore, options.sessionId)
