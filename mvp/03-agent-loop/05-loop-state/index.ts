import { tools } from "../../02-tool/07-unified-tool-interface/tools.js"
import type {
  AssistantMessage,
  Tool,
  ToolCall,
} from "../../02-tool/07-unified-tool-interface/types.js"
import {
  createInitialLoopState,
  type LoopState,
} from "./state.js"

type LoopDecision =
  | {
      type: "continue"
      reason: "tool_call"
      toolCall: ToolCall
    }
  | {
      type: "done"
      reason: "final_answer"
      content: string
    }

const apiKey = process.env.MODEL_API_KEY?.trim()
const baseUrl = (process.env.MODEL_BASE_URL ?? "https://api.deepseek.com").replace(/\/+$/, "")
const model = process.env.MODEL_NAME?.trim() || "deepseek-flash"
const userPrompt =
  process.argv.slice(2).join(" ").trim() ||
  "请先读取 package.json 找到项目名称，再获取当前服务器时间，最后告诉我项目名称和当前时间。每次只调用一个工具。"

const maxSteps = parsePositiveInteger(process.env.AGENT_MAX_STEPS ?? "4", "AGENT_MAX_STEPS")

if (!apiKey) {
  throw new Error("MODEL_API_KEY is required. Fill it in .env first.")
}

function parsePositiveInteger(rawValue: string, name: string): number {
  const value = Number(rawValue)

  if (!Number.isInteger(value) || value <= 0) {
    throw new Error(`${name} must be a positive integer, received: ${rawValue}`)
  }

  return value
}

function createToolRegistry(toolList: Tool[]): Map<string, Tool> {
  const registry = new Map<string, Tool>()

  for (const tool of toolList) {
    if (registry.has(tool.name)) {
      throw new Error(`Duplicate tool name: ${tool.name}`)
    }

    registry.set(tool.name, tool)
  }

  return registry
}

const toolRegistry = createToolRegistry(tools)
const toolSchemas = [...toolRegistry.values()].map((tool) => ({
  type: "function",
  function: {
    name: tool.name,
    description: tool.description,
    parameters: tool.parameters,
  },
}))

async function callDeepSeek(
  messages: Array<Record<string, unknown>>,
): Promise<{
  finishReason?: string | null
  message: AssistantMessage
}> {
  const response = await fetch(`${baseUrl}/chat/completions`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${apiKey}`,
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      model,
      messages,
      tools: toolSchemas,
      tool_choice: "auto",
      stream: false,
    }),
  })

  const rawBody = await response.text()
  if (!response.ok) {
    throw new Error(`DeepSeek HTTP ${response.status}: ${rawBody}`)
  }

  const payload = JSON.parse(rawBody) as {
    choices?: Array<{
      finish_reason?: string | null
      message?: AssistantMessage
    }>
  }

  const choice = payload.choices?.[0]
  if (!choice?.message) {
    throw new Error("DeepSeek returned no choices[0].message")
  }

  return {
    finishReason: choice.finish_reason,
    message: choice.message,
  }
}

function decideNextStep(message: AssistantMessage): LoopDecision {
  const toolCalls = message.tool_calls ?? []

  if (toolCalls.length > 1) {
    throw new Error(
      `agent-loop:05 intentionally supports exactly one tool call per step, but received ${toolCalls.length}`,
    )
  }

  if (toolCalls.length === 1) {
    return {
      type: "continue",
      reason: "tool_call",
      toolCall: toolCalls[0]!,
    }
  }

  const content = message.content?.trim()
  if (content) {
    return {
      type: "done",
      reason: "final_answer",
      content,
    }
  }

  throw new Error("Invalid model response: no tool_calls and no final assistant content.")
}

function printState(label: string, state: LoopState) {
  console.log(`\n[Loop State · ${label}]`)
  console.log(`step      : ${state.step}`)
  console.log(`maxSteps  : ${state.maxSteps}`)
  console.log(`messages  : ${state.messages.length}`)
  console.log(`result    : ${state.result?.reason ?? "<running>"}`)
}

const state = createInitialLoopState({
  maxSteps,
  systemPrompt:
    "你是一个简洁的助手。需要外部能力时使用工具。每次最多调用一个工具，拿到结果后再决定下一步。",
  userPrompt,
})

console.log("========== Agent Loop 05 · Loop State ==========")
console.log(`model    : ${model}`)
console.log(`user     : ${userPrompt}`)
console.log(`maxSteps : ${state.maxSteps}`)
console.log("\n这一节不增加新能力，只把运行数据统一收进 LoopState。")
console.log("Runtime Logic 负责怎么运行；LoopState 负责当前运行到哪里。")

printState("initial", state)

while (state.step < state.maxSteps) {
  state.step += 1

  console.log(`\n========== Loop Step ${state.step} / ${state.maxSteps} ==========`)
  console.log(`[LLM] messages count: ${state.messages.length}`)

  const response = await callDeepSeek(state.messages)
  console.log(`[LLM] finish_reason: ${response.finishReason ?? "N/A"}`)

  state.messages.push({
    role: "assistant",
    content: response.message.content ?? null,
    ...(response.message.tool_calls
      ? { tool_calls: response.message.tool_calls }
      : {}),
  })

  const decision = decideNextStep(response.message)
  console.log(`[Decision] type=${decision.type}, reason=${decision.reason}`)

  if (decision.type === "done") {
    state.result = {
      status: "done",
      reason: "final_answer",
      steps: state.step,
      content: decision.content,
    }
    printState("done", state)
    break
  }

  if (state.step >= state.maxSteps) {
    console.log("[Runtime] maxSteps 已耗尽，不再执行新的 Tool Call。")

    state.result = {
      status: "stopped",
      reason: "max_steps",
      steps: state.step,
    }
    printState("stopped", state)
    break
  }

  const toolCall = decision.toolCall
  const toolCallId = toolCall.id
  const name = toolCall.function?.name
  const rawArguments = toolCall.function?.arguments

  if (!toolCallId) {
    throw new Error("Tool call has no id")
  }

  if (!name) {
    throw new Error("Tool call has no function name")
  }

  if (!rawArguments) {
    throw new Error(`Tool call '${name}' has no arguments`)
  }

  console.log(`[Tool Call] name=${name}, args=${rawArguments}`)

  const tool = toolRegistry.get(name)
  if (!tool) {
    throw new Error(`Unknown tool: ${name}`)
  }

  const toolResult = await tool.execute(rawArguments)
  console.log(`[Tool Result] ${toolResult}`)

  state.messages.push({
    role: "tool",
    tool_call_id: toolCallId,
    content: toolResult,
  })

  printState("after tool", state)
  console.log("[Loop] CONTINUE → 使用同一个 LoopState 进入下一轮。")
}

if (!state.result) {
  throw new Error("Agent loop ended without state.result")
}

console.log("\n========== Agent Result ==========")
console.log(`status : ${state.result.status}`)
console.log(`reason : ${state.result.reason}`)
console.log(`steps  : ${state.result.steps}`)

if (state.result.status === "done") {
  console.log("\n[Final Assistant]")
  console.log(state.result.content)
} else {
  console.log("\nAgent 被 Runtime 的 maxSteps 边界截停。")
}

console.log("\n========== 关键观察 ==========")
console.log("agent-loop:04 的 step / maxSteps / messages / result 是散落变量。")
console.log("agent-loop:05 把它们统一收进 LoopState。")
console.log("LoopState 记录当前发生了什么；Runtime Logic 决定下一步做什么。")
console.log("行为没有变化，只是运行状态开始有了明确边界。")
console.log("下一节 agent-loop:06 会把整个执行流程进一步收成最小 runAgent() Runtime。")
